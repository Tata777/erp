<?php
	...
	//分页获取数据
	$ProResult = $objManPage->GetResult('p.`SysProductsID`, p.`ProductName`, p.`SubName`, p.`FirstPhoto`,p.`Photos`, p.`PublishDate`, p.`ValidDate`, p.`SortNum`, p.`Lang`,  s.`CateName`', '`'.con_strPREFIX.'sys_products` as p LEFT JOIN `'.con_strPREFIX.'sys_procategory` as s ON p.`CateID` = s.`SysProCategoryID`', $strSqlWhere, 10);
?>
...
<?php
	...
	$arrColumnTips = array(
										...
										array("tips" => "产品编号", "width" => "15%"),
										array("tips" => "产品名称", "width" => "20%"),
										array("tips" => "产品分类", "width" => "10%"),
	...
	$arrColumn = array(
										...
										array("Text" => "CateName"),
												...
												array(
												"Tmp" => "<img src=\"../../uploadfile/product_b/{Photos}\" width=\"120px\"  />",
													),
