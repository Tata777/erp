...
$objSort = new clsCategoryAct('linkcategory', 'link', 'CateID', 'LinkCategoryID', 'linkcateform.php');
...
<table summary="" id="pagehead" cellpadding="0" cellspacing="0" border="0" width="100%">
  <tr>
    <td width="23%"><h1><?php echo (isset($_GET['LinkCategoryID'])&&!empty($_GET['LinkCategoryID'])) ? '编辑' :'添加'?>类别</h1></td>
    <td width="77%" class="actions"><table summary="" cellpadding="0" cellspacing="0" border="0" align="right">
      <tr>
        <td ><a href="linkcatelist.php" class="view">浏览分类</a></td>
        <td class="active"><a href="linkcateform.php?ParentID=1" class="<?php echo (isset($_GET['LinkCategoryID'])&&!empty($_GET['LinkCategoryID'])) ? 'edit' :'add'?>"><?php echo (isset($_GET['LinkCategoryID'])&&!empty($_GET['LinkCategoryID'])) ? '编辑' :'添加'?>分类</a></td>
      </tr>
    </table></td>
  </tr>
</table>
...
<table cellspacing="0" cellpadding="0" width="100%"  class="maintable" id="feedbackTable">
  ...
  <tr>
    ...
    <td>
      <input type="radio" name="Lang" id="LangCn" value="cn" <?php if($arrData['list']['Lang']!='en')echo 'checked="checked"'; ?> />
      <label for="LangCn">中文</label>
      <input type="radio" name="Lang" id="LangEn" value="en" <?php if($arrData['list']['Lang']=='en')echo 'checked="checked"'; ?> />
...
<div class="buttons">
  ...
  <input type="hidden" name="act" id="act" value="<?php echo (isset($_GET['LinkCategoryID'])&&!empty($_GET['LinkCategoryID'])) ? 'edit' :'add'?>" />
  <input name="IsSelect" type="hidden" id="IsSelect" value="LinkCategoryID,ParentID,CateName,CustomLink,SortNum,Lang,IsLock,Intro" />
	<input name="IsNum" type="hidden" id="IsNum" value="LinkCategoryID,ParentID,SortNum,IsLock" />
  <?php echo (isset($_GET['LinkCategoryID'])&&!empty($_GET['LinkCategoryID'])) ? '<input name="LinkCategoryID" type="hidden" value="'.$_GET['LinkCategoryID'].'" />' :''?>
