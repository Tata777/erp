<?php
	...
	//分页获取数据
	$FeedbackResult = $objManPage->GetResult('*', '`'.con_strPREFIX.'guestbook`', "ORDER BY `guest_id` DESC", 12, true);
?>
...
?>
    ...
    <tr> 
      ...
      <th width="59%" style="text-align: left">
      	<?php echo $record['guest_title']; ?>      </th>
      <th width="18%">
...
<a href="guest_del.php?id=<? echo $record['guest_id'];?>" onClick="return confirm('你确定要删除吗？')" >删除</a>--></th>
    ...
    <tr>
    	<td><p>客户姓名:<?php echo trim($record['guest_name']);?><br />
   ...
   客户电子邮箱：<?php echo trim($record['guest_email']);?>   <br /> 
      ...
      <td colspan="2" style="vertical-align: top">
      	<p>
          发表于:
          <strong><?php echo $record['guest_date'];?></strong>
        </p>
      留言内容：  <?php echo $objBase->strTextareaConvert($record['message']);?>
      </td>
    </tr>
								<?php
								if($record['guest_re']!=""){
