...
<script language="javascript">
	...
	if(!check_empty(dom.pay,"支付类弄不能为空！")) return false;
		return true;
	 }
...
<div id="center2">
  ...
  <div class="page_right_4 new_t">
	...
	foreach($vm as $key=>$value){
	 ?>
        ...
        <div class="news_pic02">
          ...
          <div class="news_pic03"><?=$value['ProductName']?><span class="red">【<?=$value['Price']?>元】</span> </div>
	...
	?>
	  ...
	  <div class="STYLE2">&nbsp;&nbsp;温馨提示：请确认你以下的资料是否正确,正确请选择支付类型直接提交。不正确请点击返回上一步进行修改重新提交！</div>
	    <form id="messageform" name="messageform" method="post" action="contactorder_ok.php" >
        <input name="action" type="hidden" value="guestmessage" />
		 <input name="prid" type="hidden" value="<?=$_POST['prid']?>" />
      <table cellspacing="0" align="center" cellpadding="3" width="98%" style="margin-top:20px;">
        <tr>
          ...
          <td width="527" align="left" class="news"><input name="guest_name" type="hidden" value="<?=$_POST['guest_name']?>" /><?=$_POST['guest_name']?></td>
        </tr>
        <tr>
          <td class="news"><div align="right"> 客户性别：</div></td>
          <td align="left" class="news">
			 <input name="guest_sex" type="hidden" value="<?=$_POST['guest_sex']?>" /><?=$_POST['guest_sex']?>
			</td>
        </tr>
        <tr>
          <td class="news"><div align="right"> 电子邮箱：</div></td>
          <td align="left" class="news">	<input name="guest_email" type="hidden" value="<?=$_POST['guest_email']?>" /> <?=$_POST['guest_email']?>
           </td>
        </tr>
        <tr>
          <td class="news"><div align="right"> 手机号码：</div></td>
          <td align="left" class="news">  <input name="guest_contact" type="hidden" value="<?=$_POST['guest_contact']?>" /><?=$_POST['guest_contact']?>
           </td>
        </tr>
        ...
        <tr>
          ...
          <td align="left" class="news">
         <input name="guest_start" type="hidden" value="<?=$_POST['guest_start']?>" /> <?=$_POST['guest_start']?></td>
        </tr>
        <tr>
          <td class="news"><div align="right"> 离开时间：</div></td>
          <td align="left" class="news">
		
		<input name="guest_end" type="hidden" value="<?=$_POST['guest_end']?>" />  <?=$_POST['guest_end']?>
           </td>
        </tr>
        <tr>
          <td class="news"><div align="right"> 预定房型：</div></td>
          <td align="left" class="news"><input name="guest_fuwu" type="hidden" value="<?=$_POST['guest_fuwu']?>" /><?=$_POST['guest_fuwu']?></td>
        </tr>
        <tr>
          <td class="news"><div align="right"> 预定数量：</div></td>
          <td align="left" class="news"><input name="guest_num" type="hidden" value="<?=$_POST['guest_num']?>" /><?=$_POST['guest_num']?></td>
        </tr> 
		 <tr>
          <td class="news"><div align="right"> 共需支付金额：</div></td>
          <td align="left" class="news"><input name="houseprice" type="hidden" value="<?=$_POST['houseprice']*$_POST['guest_num']?>" /><?=$_POST['houseprice']*$_POST['guest_num']?></td>
        </tr>
        <tr>
          <td valign="top" class="news"><div align="right"> 备注内容：</div></td>
          <td align="left" class="news"><input name="guest_content" type="hidden" value="<?=$_POST['guest_content']?>" /><?=$_POST['guest_content']?><br /></td>
        </tr>
          <tr>
          <td valign="top" class="news"><div align="right"> 支付方式：</div></td>
          <td align="left" class="news"><label>
            <input type="radio" name="pay" value="alipy"   checked="checked" />
            支付宝支付
            <input type="radio" name="pay" value="offline" />
            线下支付</label></td>
        </tr>
        
        <tr>
          <td class="news"><div align="right"></div></td>
          <td align="left" class="news"><input type="submit" name="button" id="button" value="提交预定" /><input type="button" onclick="history.go(-1);" name="button" id="button" value="返回上一步" /></td>
        </tr>
      </table>
	  </form>
    </div>
