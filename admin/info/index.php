<?php
	...
	{
		$strSqlWhere = ($_GET['con']) ? $_GET['con'] : " ORDER BY i.`InfoID` DESC";
	}
	...
	//分页获取数据
	$InfoResult = $objManPage->GetResult('i.`InfoID`,i.`TopNum`, i.`Title`,i.`FirstPhoto`, i.`Role`, i.`Recommand`, i.`SortNum`, i.`Lang`, i.`PublishDate`, s.`CateName` , a.`ad_username`', '`'.con_strPREFIX.'info` as i LEFT JOIN `'.con_strPREFIX.'infocategory` as s ON i.`CateID` = s.`InfoCategoryID` LEFT JOIN `admin` AS a ON i.`ModifiedUserID` = a.`ad_id`', $strSqlWhere, 20);
/*echo "<pre>";
var_dump($InfoResult);
echo "</pre>";*/
?>
...
<?php
	...
	$arrColumnTips = array(
										...
										array("tips" => "排序", "width" => "10%"),
										array("tips" => "标题", "width" => "20%"),
										array("tips" => "信息分类", "width" => "10%"),
										array("tips" => "编辑", "width" => "10%"),
										array("tips" => "发布日期", "width" => "10%"),
										array("tips" => "推荐", "width" => "10%"),
                                         array("tips" => "置顶值", "width" => "10%"),
										array("tips" => "语言", "width" => "10%"),
										array("tips" => "操作", "width" => "10%")
									 );
									 
	$arrColumn = array(
										array("CheckBox" => "InfoID"),
										array("Text" => "InfoID"),
										array("Sort" => "SortNum"),
										"Title" => 
										array(
										...
										array("Text" => "CateName"),
										array("Text" => "ad_username"),
										array("Date" => "PublishDate"),
										array("TickOff" => "Recommand"),
										array("Text" => "TopNum"),
										array("LangSelect" => "Lang"),
										array("Tmp" => "<sapn class=\"buttons\"><input type=\"button\" value=\"编辑\" class=\"submit\" onclick=\"PopupWindow('info.php?act=modify&id={InfoID}', 800, 600)\"></span>")
									 );
	...
	$arrOperation = array ("RevChk", "Sort","PublishDate", "Copy", "Move","Recommand","Top", "Del");
	$objBO->strCateTable = "infocategory";
