...
class Info extends mysqldb
{
   ...
   {
	  	$SQL="SELECT * FROM `hy_b_info` where CateID='$cateid' and Lang='fn' limit 1 ";
        $result=$this->commonQuery($SQL);
	  	 return $result;
   }
   ...
   function getNewsOne($id)
   {
		...
		parent::query($SQL);
	  	$SQL="SELECT * FROM `hy_b_info` where InfoID='$id' and Lang='fn' ";
        $result=$this->commonQuery($SQL);
	  	 return $result;
   }
   ...
   {
	   	$SQL="SELECT * FROM `hy_b_info` where CateID='$cateid' and Lang='fn' and $limit  ";
        $result=$this->commonQuery($SQL);
	  	return $result;
   }
  ...
  {
		 $countSql="SELECT * FROM hy_b_info  where Lang='fn'  and `CateID`=".$infocateid; //sql语句
	   ...
	   // $urlPara="id=".$id."&cateid=".$cid;//这是URL后面跟的参数，也就是问号传值
		...
		$sql = "select * FROM  hy_b_info where   Lang='fn'  and CateID ='$infocateid' order by InfoID DESC limit $rsStart, $pageSize";
		//echo $sql;
		//$res = mysql_query($sql);
		$objDb = new mysqldb();
		$objDb->query($sql);
   ...
   */
   ...
   {
	   	$SQL="SELECT * FROM `hy_b_sys_products_order` where Lang='fn'  and `CateID`='$procateid' and Unit>0 limit 0, $limit";//sql语句
         $result=$this->commonQuery($SQL);
	  	return $result;
   }
        /* 获取产品多条信息 */
   ...
   {
	   	$SQL="SELECT * FROM `hy_b_sys_products` where Lang='fn'  and `CateID`='$procateid' limit 0, $limit";//sql语句
         $result=$this->commonQuery($SQL);
	  	return $result;
   }
	  ...
	  function getProductList($procateid){
             ...
             $countSql="SELECT * FROM  `hy_b_sys_products` where Lang='fn'  and `CateID`=".$procateid;//sql语句
            $pageSize="12"; //每页显示数
            $curPage=@$_GET['curPage'];//通过GET传来的当前页数
           // $urlPara="id=".$id."&cid=".$cid;//这是URL后面跟的参数，也就是问号传值
            ...
            $pageStr=$pageOutStrArr[2];//limit 后的第二个参数
            $pageCount=$pageOutStrArr[1];//总页数
            $sql = "SELECT * FROM  hy_b_sys_products  where   Lang='fn'  and `CateID` ='$procateid' order by SysProductsID DESC limit $rsStart, $pageSize";
            //$res = mysql_query($sql);
			$objDb = new mysqldb();
            $objDb->query($sql);
            $row1=$objDb->get_data();
	  ...
	  function getProductOrderList($procateid){
             ...
             $countSql="SELECT * FROM  `hy_b_sys_products_order` where Lang='fn' and Unit>0  and `CateID`=".$procateid;//sql语句
            $pageSize="12"; //每页显示数
            $curPage=@$_GET['curPage'];//通过GET传来的当前页数
           // $urlPara="id=".$id."&cid=".$cid;//这是URL后面跟的参数，也就是问号传值
            ...
            $pageStr=$pageOutStrArr[2];//limit 后的第二个参数
            $pageCount=$pageOutStrArr[1];//总页数
            $sql = "SELECT * FROM  hy_b_sys_products_order  where   Lang='fn' and Unit>0  and `CateID` ='$procateid' order by SysProductsID DESC limit $rsStart, $pageSize";
            //$res = mysql_query($sql);
			$objDb = new mysqldb();
            $objDb->query($sql);
            $row1=$objDb->get_data();
