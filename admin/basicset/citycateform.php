...
$objSort = new clsCategoryAct('city', 'member', 'CmpCity', 'CityID', 'citycateform.php');
...
<table summary="" id="pagehead" cellpadding="0" cellspacing="0" border="0" width="100%">
  <tr>
    <td width="23%"><h1><?php echo (isset($_GET['CityID'])&&!empty($_GET['CityID'])) ? '编辑' :'添加'?>类别</h1></td>
    <td width="77%" class="actions"><table summary="" cellpadding="0" cellspacing="0" border="0" align="right">
      <tr>
        <td ><a href="citycatelist.php" class="view">浏览分类</a></td>
        <td class="active"><a href="citycateform.php?ParentID=1" class="<?php echo (isset($_GET['CityID'])&&!empty($_GET['CityID'])) ? 'edit' :'add'?>"><?php echo (isset($_GET['CityID'])&&!empty($_GET['CityID'])) ? '编辑' :'添加'?>分类</a></td>
      </tr>
    </table></td>
  </tr>
</table>
...
<div class="buttons">
  ...
  <input type="hidden" name="act" id="act" value="<?php echo (isset($_GET['CityID'])&&!empty($_GET['CityID'])) ? 'edit' :'add'?>" />
  <input name="IsSelect" type="hidden" id="IsSelect" value="CityID,ParentID,CateName,CustomLink,SortNum,IsLock,Intro" />
	<input name="IsNum" type="hidden" id="IsNum" value="CityID,ParentID,SortNum,IsLock" />
  <?php echo (isset($_GET['CityID'])&&!empty($_GET['CityID'])) ? '<input name="CityID" type="hidden" value="'.$_GET['CityID'].'" />' :''?>
